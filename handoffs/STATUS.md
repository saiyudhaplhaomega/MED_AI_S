...
- 13:00 done: Workspace landing, sample uploads, local past cases, filters, virtual event list, drawers, key events, and accident milestone are implemented; build passes.
  - 13:42 done: AI case brief, Q&A citations, event rephrase, and lazy headlines added with offline fallbacks.
  - 13:50 done: PPT export, unseen 300-row fixture, and requested diagnostic endpoint removal completed.
...
- 13:58 done: AI engines settled. Gemini (gemini-flash-latest) is the primary engine for /api/ai and the QA fallback; user MiniMax key is out of platform credits (Token Plan 2056), kept as last fallback. n8n Case QA chatbot restored to its working version and verified live. AI Gateway workflow archived (broken credential binding). DO NOT republish the Case QA Chatbot workflow in n8n: only its current active version has the working n8n-credits binding. Floor verified live: Middleswarth loads on the deployed URL with phases, stats, key events.
